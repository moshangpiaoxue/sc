package com.mo.lib.utils.systemUtils.fingerUtil;
...
public class FingerDentifyManager {
    ...
    public boolean isBiometricPromptEnable(boolean isShow) {
        if (isShow) {
            if (!isAboveApi23()) {
                new BaseIosDialog(mActivity).setMsg("您的系统版本过低不支持指纹登录，请取消").setLeftTextView("取消", null)
                        .setDismissType(BaseDialog.DismissType.OTHER) .show();
                return false;
            } else if (!isHardwareDetected()) {
                new BaseIosDialog(mActivity).setMsg("您的设备不支持指纹登录，请取消").setLeftTextView("取消", null)
                        .setDismissType(BaseDialog.DismissType.OTHER) .show();
                return false;
            } else if (!hasEnrolledFingerprints()) {
                new BaseIosDialog(mActivity).setMsg("您还没有录入指纹, 请在手机设置界面录入至少一个指纹").setLeftTextView("取消", null)
                        .setDismissType(BaseDialog.DismissType.OTHER) .show();
                return false;
            } else if (!hasEnrolledFingerprints()) {
                new BaseIosDialog(mActivity).setMsg("您还没有设置锁屏密码, 请在手机设置界面设置").setLeftTextView("取消", null)
                        .setDismissType(BaseDialog.DismissType.OTHER) .show();
                return false;
            } else if (isMuthError) {
                new BaseIosDialog(mActivity).setMsg("您的指纹验证多次错误，请稍后再试").setLeftTextView("取消", null)
                        .setDismissType(BaseDialog.DismissType.OTHER) .show();
                return false;
            } else {
                return true;
