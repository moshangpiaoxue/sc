...
import com.mo.lib.utils.dataUtil.KUriUtil;
import com.mo.lib.utils.hardware_utils.CameraUtil;
import com.mo.lib.utils.image.BitmapCompressUtil;
...
public interface MediaAction {
    ...
    default void actionMediaTakePic() {
        if (CameraUtil.isExistCamera()) {
            ...
            XXPermissions.with((Activity) getContext()).permission(Permission.CAMERA).request(new KOnPermission() {
                @Override
    ...
    default void actionMediaChoosePic() {
        if (SDCardUtil.isEnable()) {
            ...
            XXPermissions.with((Activity) getContext()).permission(Permission.Group.STORAGE).request(new KOnPermission() {
                @Override
                public void hasPermission(List<String> granted, boolean all) {
                    if (all){
